...
interface TurfXLogoProps {
  size?: 'small' | 'medium' | 'large';
  className?: string;
}

export const TurfXLogo: React.FC<TurfXLogoProps> = ({ size = 'medium', className = '' }) => {
  // Size configurations
  const sizeConfig = {
    small: {
      text: 'text-3xl',
  ...
  const footballSize = size === 'small' ? 'w-10 h-10' : size === 'medium' ? 'w-14 h-14' : 'w-20 h-20';

  return (
    <div className={`relative ${className}`}>
      <div className="flex items-center gap-2">
        {/* Football Logo on Left */}
        <FootballLogo 
          className={footballSize} 
...
// Original style logo (large version)
export function OriginalStyleLogo() {
  return <TurfXLogo size="large" />;
}
